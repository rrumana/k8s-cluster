...
- The retained target claim is `media/media-library-bulk`, PV
  ...
  `Retain`.
- `media-library-cephfs-bulk-seed-v8` is the active online seed. It mounts the
  ...
  56 MiB/s per stream. Every name in a hard-link group therefore stays in one
...
- The source has 2,709 hard-link groups spanning top-level branches, including
  ...
  The v8 device-and-inode assignment measured 1.28-1.45 TB of unique data per
...
- The new EC pool had about 8.2 TiB maximum available. Coexistence is projected
  ...
  threshold is 85%.
- The resumed concurrent seed has a 224 MiB/s combined logical cap across four
  inode-disjoint 56 MiB/s streams. The preceding 256 MiB/s cap reached the
  requested aggregate rate, but OSD.0 then recorded 5-7 second BlueStore
  commit stalls and an actual SMART thermal-management level-2 transition.
  This 12.5% reduction is intended to retain roughly 230-260 MiB/s aggregate
  reads and writes with the current background I/O. It may run alongside
